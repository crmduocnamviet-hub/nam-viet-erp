...
export const getLabOrders = async (filters?: {
  ...
  let query = supabase.from(TABLES.LAB_ORDERS).select(`
      *,
  ...
  if (filters?.visitId) {
    query = query.eq("medical_visits.visit_id", filters.visitId);
  }
  ...
  if (filters?.startDate || filters?.endDate) {
    let visitQuery = supabase.from(TABLES.MEDICAL_VISITS).select("visit_id");
