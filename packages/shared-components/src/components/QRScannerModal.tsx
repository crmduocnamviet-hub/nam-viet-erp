  ...
  const startCamera = async () => {
    try {
      ...
      if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
        setError("Camera chỉ hoạt động trên HTTPS. Vui lòng truy cập qua HTTPS.");
        return;
      }
      ...
      if (!hasDetector) {
        setError("Trình duyệt không hỗ trợ quét mã barcode. Vui lòng sử dụng Chrome hoặc Edge phiên bản mới nhất.");
      ...
      if (err.name === "NotAllowedError") {
        setError("Quyền truy cập camera bị từ chối. Vui lòng cấp quyền camera.");
        setShowTutorial(true);
